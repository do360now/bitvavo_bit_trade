...
class FeatureEngineer:
    ...
    def _extract_sentiment_features(self, data: Dict) -> Tuple[List[float], List[str]]:
        ...
        names.extend(['sentiment_normalized', 'sentiment_positive', 'sentiment_negative'])
        ...
        if model_type == ModelType.RANDOM_FOREST:
            ...
            if is_classification:
                return MLPClassifier(
                    hidden_layer_sizes=config.model_params.get('hidden_layers', (100, 50)),
...
class MLEngine:
        ...
        if self.config.model_type == ModelType.ENSEMBLE:
            ...
            if is_classification:
                self.model = VotingClassifier(
                    estimators=list(self.ensemble_models.items()),
                    voting='soft'
                )
            else:
    ...
    def _create_label(self, current_row: pd.Series, next_row: pd.Series, task: PredictionTask):
    ...
    def train(self, training_data: Union[pd.DataFrame, Tuple[np.ndarray, np.ndarray]]):
        ...
        if self.config.model_type == ModelType.ENSEMBLE and isinstance(self.model, dict):
        ...
        logger.info(f"Model trained in {training_time:.2f}s - Accuracy: {performance.accuracy:.3f}, F1: {performance.f1_score:.3f}")
    ...
    def _select_features(self, X: np.ndarray, y: np.ndarray) -> np.ndarray:
        ...
        selected_names = [self.feature_engineer.feature_names[i] for i in selected_features]
    ...
    def predict(self, market_data: Dict[str, Any]) -> Tuple[Any, float]:
        ...
        if (self.config.online_learning_enabled and 
            self.prediction_count - self.last_retrain_count >= self.config.retraining_interval):
            self._trigger_retrain()
        
        logger.debug(f"Prediction: {prediction}, Confidence: {confidence:.3f}, Time: {prediction_time:.4f}s")
    ...
    def _ensemble_predict(self, X: np.ndarray) -> np.ndarray:
        ...
        if self.config.prediction_task in [PredictionTask.PRICE_DIRECTION, PredictionTask.TRADE_SUCCESS, PredictionTask.OPTIMAL_ACTION]:
            # Majority voting
    ...
    def get_performance_summary(self) -> Dict[str, Any]:
        ...
        return {
            ...
            'is_trained': self.is_trained,
            'top_features': list(self.feature_importance.items())[:5] if self.feature_importance else []
        }
