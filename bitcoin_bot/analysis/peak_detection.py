...
class PeakDetector:
    ...
    def __init__(self, config: Optional[PeakDetectionConfig] = None):
        ...
        logger.info(
            f"Peak detector initialized with lookback={self.config.lookback_periods}"
        )
    ...
    def _detect_scipy_peaks(self, prices: np.ndarray) -> List[PeakPattern]:
        ...
        for idx in peak_indices:
            confidence = self._calculate_peak_confidence(
                ...
                idx,
                (
                    properties.get("prominences", [0])[0]
    ...
    def _detect_local_extrema(self, prices: np.ndarray) -> List[PeakPattern]:
        ...
        local_max = argrelextrema(
            prices, np.greater, order=self.config.min_distance_periods // 2
        )[0]
        ...
        for idx in outlier_indices:
            if idx > 0 and idx < len(prices) - 1:
                if prices[idx] > prices[idx - 1] and prices[idx] > prices[idx + 1]:
                    confidence = min(
                        0.95, (z_scores[idx] - self.config.zscore_threshold) / 2 + 0.5
                    )
    ...
    def _find_double_tops(self, prices: np.ndarray) -> List[PeakPattern]:
        ...
        for i in range(len(peaks) - 1):
            ...
            for j in range(
                i + 1, min(i + 3, len(peaks))
            ):  # Look for second peak within reasonable distance
                second_peak = peaks[j]

                # Check if peaks are similar height (within 2%)
                height_diff = (
                    abs(prices[first_peak] - prices[second_peak]) / prices[first_peak]
                )

                if height_diff < 0.02:
                    # Find valley between peaks
                    valley_idx = np.argmin(prices[first_peak:second_peak]) + first_peak
                    valley_depth = (prices[first_peak] - prices[valley_idx]) / prices[
                        first_peak
                    ]
    ...
    def _find_blow_off_tops(
        ...
        for peak_idx in peaks:
            ...
            if (
                price_gain > 0.10  # 10% gain in 10 periods
                and acceleration > 0
                and vol_spike > self.config.volume_spike_threshold
            ):
    ...
    def _calculate_peak_confidence(
        self, prices: np.ndarray, peak_idx: int, prominence: float = 0
    ) -> float:
        ...
        if len(local_prices) > 0:
            height_ratio = (
                (prices[peak_idx] - np.mean(local_prices)) / np.std(local_prices)
                if np.std(local_prices) > 0
                else 0
            )
        ...
        try:
            ...
            for pattern in patterns:
                pattern_dict = {
                    ...
                    "timestamp": (
                        pattern.peak_time.isoformat() if pattern.peak_time else None
                    ),
                }
            ...
            if len(self.pattern_database) > self.config.max_stored_patterns:
                self.pattern_database = self.pattern_database[
                    -self.config.max_stored_patterns :
                ]
    ...
    def get_nearest_levels(self, current_price: float) -> Dict[str, Optional[float]]:
        ...
        if resistances_above:
            ...
            result["distance_to_resistance"] = (
                result["nearest_resistance"] - current_price
            ) / current_price
        ...
        if supports_below:
            ...
            result["distance_to_support"] = (
                current_price - result["nearest_support"]
            ) / current_price
...
class PeakProbabilityCalculator:
    ...
    def calculate_peak_probability(
        ...
        recent_peaks = self.detector.detect_peaks(
            prices[-50:] if len(prices) >= 50 else prices
        )
        if recent_peaks:
            ...
            if latest_peak.peak_index >= len(prices) - 5:  # Peak in last 5 periods
                ...
                reasons.append(
                    f"Recent peak detected (conf: {latest_peak.confidence:.2f})"
                )
        ...
        if probabilities:
            ...
            weights = [
                p**2 for p in probabilities
            ]  # Square to emphasize strong signals
    ...
    def _analyze_volume_divergence(
        self, prices: np.ndarray, volumes: np.ndarray
    ) -> float:
        ...
        volume_trend = (
            (recent_volumes[-1] - recent_volumes[0]) / recent_volumes[0]
...
class PeakAvoidanceSystem:
    ...
    def analyze(
        ...
        # Detect peaks
        detected_peaks = self.detector.detect_peaks(
            prices_array, volumes_array, timestamps
        )
        ...
        if self.last_peak_time:
            time_since_peak = (
                datetime.now() - self.last_peak_time
            ).total_seconds() / 60  # Minutes
            if (
                time_since_peak < self.config.cooldown_periods_after_peak * 15
            ):  # Assuming 15-min periods
                in_cooldown = True

        # Generate final recommendation
        should_avoid = (
            peak_analysis["peak_probability"] > self.config.peak_avoidance_threshold
            or self.consecutive_peak_signals >= 3
            or in_cooldown
        )
...
if __name__ == "__main__":
    ...
    # Generate timestamps
    timestamps = [
        datetime.now() - timedelta(minutes=15 * i) for i in range(periods - 1, -1, -1)
    ]
