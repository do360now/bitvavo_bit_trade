...
class TradingBot:
        ...
        try:
            ...
            if os.path.exists(self.data_manager.bot_logs_file):
                ...
                if len(df) >= self.config.ml_min_training_samples:
                    ...
                    logger.info(f"Insufficient data for ML training ({len(df)} samples)")
        ...
        try:
            ...
            if len(prices) > 100:
                self._peak_system.analyze_price_history(
                    prices[-500:], 
                    volumes[-500:],
                    [datetime.now() - timedelta(minutes=15*i) for i in range(min(500, len(prices)))][::-1],
                    [{}] * min(500, len(prices))
                )
    ...
    def analyze_market(self) -> MarketIndicators:
        ...
        upper, middle, lower = calculate_bollinger_bands(prices) or (current_price, current_price, current_price)
        ...
        logger.debug(f"Market analysis complete: Price={current_price:.2f}, RSI={rsi:.1f}, Volatility={volatility:.4f}")
    ...
    def _calculate_volatility_safe(self, prices, window=20):
        ...
        try:
            ...
            recent_prices = np.array(prices[-window-1:])  # Need window+1 prices to get window returns
            ...
            returns[valid_mask] = price_changes[valid_mask] / previous_prices[valid_mask]
    ...
    def _determine_market_regime(self, prices: List[float], ma_short: float, ma_long: float) -> str:
    ...
    def _add_sentiment_analysis(self, indicators: MarketIndicators):
        ...
        try:
            from core.indicators import fetch_enhanced_news, calculate_enhanced_sentiment
            ...
            if articles:
                ...
                indicators.risk_off_probability = analysis.get('risk_off_probability', 0.0)
                logger.debug(f"Sentiment: {indicators.sentiment:.3f}, Risk-off: {indicators.risk_off_probability:.3f}")
        except Exception as e:
    ...
    def _add_ml_predictions(self, indicators: MarketIndicators):
        ...
        try:
            indicators_dict = {
                ...
                'news_analysis': {'risk_off_probability': indicators.risk_off_probability}
            }
            
            fail_prob, success_prob = self._ml_engine.predict_trade_success(indicators_dict)
            indicators.ml_success_probability = success_prob
            logger.debug(f"ML prediction: {success_prob:.3f} success probability")
        except Exception as e:
            logger.warning(f"ML prediction failed: {e}")
    
    def _add_peak_analysis(self, indicators: MarketIndicators, prices: List[float], volumes: List[float]):
        """Add peak detection analysis to indicators"""
        try:
            indicators_dict = {
                'rsi': indicators.rsi,
                'macd': indicators.macd,
                'volume_ratio': indicators.current_volume / np.mean(volumes[-10:]) if len(volumes) >= 10 else 1.0
            }
            ...
            indicators.peak_recommendation = analysis.get('recommended_action', 'neutral')
    ...
    def _add_onchain_analysis(self, indicators: MarketIndicators):
        ...
        try:
            ...
            logger.debug(f"On-chain: Netflow={indicators.netflow:.2f}, FeeRate={indicators.fee_rate:.2f}")
        except Exception as e:
    ...
    def generate_signal(self, indicators: MarketIndicators) -> TradingSignal:
        ...
        # Normal trading logic
            ...
            if buy_score >= 4 and risk_level != RiskLevel.EXTREME:
                ...
                action = TradingAction.HOLD
                confidence = 0.5
                reasoning = [f"Insufficient signals: Buy={buy_score}/7, Sell={sell_score}/7"]
        ...
        volume = self._calculate_position_size(action, indicators, confidence, risk_level)
        ...
        logger.info(f"Signal generated: {action.value.upper()} with {confidence:.1%} confidence")
    ...
    def _check_emergency_conditions(self, indicators: MarketIndicators) -> Optional[TradingAction]:
        """Check for emergency trading conditions"""
        # Extreme risk-off
        if indicators.risk_off_probability > 0.8:
            logger.warning(f"EMERGENCY: Extreme risk-off probability {indicators.risk_off_probability:.1%}")
            return TradingAction.SELL
        
        # Liquidation cascade detection
        if (indicators.volatility > 0.10 and 
            indicators.sentiment < -0.3 and 
            indicators.current_price < indicators.vwap * 0.95):
    ...
    def _calculate_sell_score(self, indicators: MarketIndicators) -> Tuple[float, List[str]]:
        ...
        if indicators.ml_success_probability and indicators.ml_success_probability < 0.3:
            score += 1
            reasons.append(f"Low ML confidence: {indicators.ml_success_probability:.1%}")
        
        if indicators.peak_probability and indicators.peak_probability > self.config.peak_avoid_threshold:
            score += 1.5
    ...
    def _apply_peak_override(
        ...
        if (action == TradingAction.BUY and 
            indicators.peak_probability > self.config.peak_avoid_threshold):
            
            logger.warning(f"PEAK OVERRIDE: Changing BUY to HOLD (peak prob: {indicators.peak_probability:.1%})")
            return TradingAction.HOLD, confidence * 0.5, ["Peak detection override"] + reasoning
        
        # Enhance sell signal if at peak
        if (action == TradingAction.SELL and 
            indicators.peak_probability > self.config.peak_avoid_threshold):
            
            confidence = min(0.95, confidence * 1.2)
            reasoning = [f"Peak detected: {indicators.peak_probability:.1%}"] + reasoning
    ...
    def _calculate_position_size(
        ...
        # Apply bounds
        position_pct = max(self.config.min_position_size_pct, 
    ...
    def execute_strategy(self):
        ...
        try:
            ...
            if (self.config.enable_ml and 
                self.total_trades % self.config.ml_retrain_interval_trades == 0):
    ...
    def _can_trade(self) -> bool:
        ...
        if time.time() - self.last_trade_time < self.config.trade_cooldown_seconds:
            remaining = self.config.trade_cooldown_seconds - (time.time() - self.last_trade_time)
    ...
    def _log_trading_decision(self, signal: TradingSignal, indicators: MarketIndicators):
        """Log trading decision for analysis"""
        try:
            self.data_manager.log_strategy(
                timestamp=signal.timestamp.isoformat(),
                price=indicators.current_price,
                trade_volume=signal.volume if signal.action != TradingAction.HOLD else 0,
    ...
    def get_status(self) -> Dict[str, Any]:
        ...
        try:
            ...
            if self._performance_tracker:
                status['performance'] = self._performance_tracker.generate_performance_report()
