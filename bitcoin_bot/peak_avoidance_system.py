...
class PeakAvoidanceSystem:
    ...
    def _load_historical_patterns(self):
        ...
        try:
            if os.path.exists(self.pattern_file):
                ...
                print(f"✅ Loaded {len(self.pattern_database)} historical peak patterns")
    ...
    def analyze_price_history(self, prices: List[float], volumes: List[float], 
                            timestamps: List[datetime], indicators: List[Dict]) -> List[PeakPattern]:
        """
        ...
        for peak_idx in peaks:
            ...
            peak_time = timestamps[peak_idx] if peak_idx < len(timestamps) else datetime.now()
            ...
            if drop_percentage > 0.03:  # 3% or more drop
                ...
                if peak_idx < len(indicators):
                    ...
                    leading_indicators = {
                        ...
                        'volume_ratio': volumes[peak_idx] / np.mean(volumes[max(0, peak_idx-10):peak_idx]) if peak_idx > 10 else 1.0,
                        'price_momentum': (prices[peak_idx] - prices[peak_idx-5]) / prices[peak_idx-5] if peak_idx > 5 else 0
                    }
                
                # Classify peak type
                pattern_type = self._classify_peak_type(drop_percentage, time_to_drop, leading_indicators)
        ...
        self.pattern_database.extend([self._pattern_to_dict(p) for p in detected_patterns])
    ...
    def _detect_peaks_multi_method(self, prices: np.array) -> List[int]:
        ...
        peaks1, properties = find_peaks(prices, prominence=prices.std() * 0.5, distance=5)
        ...
        for peak in peaks_unique:
            if not filtered_peaks or peak - filtered_peaks[-1] > 5:  # At least 5 periods apart
                filtered_peaks.append(peak)
        
        return filtered_peaks
    
    def _classify_peak_type(self, drop_percentage: float, time_to_drop: int, 
        ...
        if (drop_percentage > 0.08 and rsi > 75 and 
            volume_ratio > 2.0 and time_to_drop < 60):
            return 'blow_off_top'
    ...
    def _pattern_to_dict(self, pattern: PeakPattern) -> Dict:
        ...
        return {
            ...
            'peak_time': pattern.peak_time.isoformat() if isinstance(pattern.peak_time, datetime) else str(pattern.peak_time),
    ...
    def _update_peak_indicators(self):
        ...
        for ptype, patterns in by_type.items():
            self.peak_indicators[ptype] = {
                ...
                'avg_rsi_at_peak': np.mean([p['leading_indicators'].get('rsi', 50) for p in patterns]),
                'avg_volume_ratio': np.mean([p['leading_indicators'].get('volume_ratio', 1.0) for p in patterns])
            }
    
    def predict_peak_probability(self, current_price: float, current_indicators: Dict[str, float],
                               recent_prices: List[float], recent_volumes: List[float]) -> Dict[str, float]:
        """
        ...
        if len(recent_prices) < 20 or not self.pattern_database:
            return {'peak_probability': 0.3, 'confidence': 0.0, 'recommended_action': 'neutral'}
        ...
        current_volume_ratio = (recent_volumes[-1] / np.mean(recent_volumes[-10:])) if len(recent_volumes) >= 10 else 1.0
        current_momentum = (current_price - recent_prices[-5]) / recent_prices[-5] if len(recent_prices) >= 5 else 0
        
        # Check if we're at a local high
        is_local_high = current_price >= max(recent_prices[-10:]) if len(recent_prices) >= 10 else False
        ...
        for pattern_type in ['blow_off_top', 'major_peak', 'local_peak']:
            prob = self._calculate_pattern_similarity(current_characteristics, pattern_type)
        ...
        confidence = min(1.0, len(self.pattern_database) / 50)  # Full confidence with 50+ patterns
        
        # Generate recommendation
        recommendation = self._generate_peak_recommendation(overall_probability, probabilities, current_characteristics)
    ...
    def _calculate_pattern_similarity(self, current_characteristics: Dict, pattern_type: str) -> float:
        """Calculate similarity between current conditions and historical peak patterns"""
        if pattern_type not in self.peak_indicators:
            return 0.0
        
        type_patterns = [p for p in self.pattern_database if p.get('pattern_type') == pattern_type]
        ...
        for pattern in type_patterns:
            ...
            rsi_similarity = 1.0 - abs(current_characteristics['rsi'] - pattern_indicators.get('rsi', 50)) / 100.0
            volume_similarity = 1.0 - min(1.0, abs(current_characteristics['volume_ratio'] - pattern_indicators.get('volume_ratio', 1.0)) / 2.0)
            momentum_similarity = 1.0 - min(1.0, abs(current_characteristics['momentum'] - pattern_indicators.get('price_momentum', 0)) / 0.1)
            
            # Weight the similarities
            overall_similarity = (rsi_similarity * 0.4 + volume_similarity * 0.3 + momentum_similarity * 0.3)
            similarities.append(max(0.0, overall_similarity))
        
        # Return average similarity to this pattern type
        return np.mean(similarities) if similarities else 0.0
    
    def _generate_peak_recommendation(self, overall_prob: float, pattern_probs: Dict, 
                                    current_chars: Dict) -> Dict[str, str]:
    ...
    def get_adaptive_entry_strategy(self, current_price: float, current_indicators: Dict,
                                  recent_prices: List[float], recent_volumes: List[float]) -> Dict:
        """
        Get adaptive entry strategy that learns from peak patterns.
        Returns specific entry recommendations and position sizing.
        """
        peak_analysis = self.predict_peak_probability(current_price, current_indicators, recent_prices, recent_volumes)
        ...
        return {
            ...
            'strategy_reasoning': self._generate_strategy_reasoning(peak_analysis, base_position_multiplier)
        }
    
    def _calculate_smart_entry_price(self, current_price: float, recent_prices: List[float],
                                   peak_analysis: Dict, max_chase_pct: float) -> float:
        ...
        if len(recent_prices) >= 20:
            ...
            if recent_lows:
                ...
                target_entry = max(support_level, current_price * (1 - 0.02))  # Don't go more than 2% below current
            else:
        ...
        if peak_prob > 0.6:
            ...
            target_entry = min(current_price * (1 + max_chase_pct), current_price * 1.01)
        
        return target_entry
    
    def _generate_strategy_reasoning(self, peak_analysis: Dict, position_multiplier: float) -> List[str]:
        ...
        reasoning.append(f"Peak probability: {peak_prob:.1%} (confidence: {confidence:.1%})")
        ...
        for pattern_type, prob in pattern_probs.items():
            if prob > 0.5:
                reasoning.append(f"Warning: {pattern_type.replace('_', ' ').title()} pattern detected ({prob:.1%})")
        ...
        if current_chars.get('volume_ratio', 1.0) > 2.0:
            reasoning.append(f"Volume spike: {current_chars['volume_ratio']:.1f}x normal")
    ...
    def _add_failed_entry_pattern(self, failed_trade: Dict):
        ...
        print(f"📚 Added failed entry pattern: {failed_trade.get('price', 0):.2f} → {failed_trade.get('loss_percentage', 0):.1%} loss")
...
class PeakAwareTrader:
    ...
    def should_avoid_buying(self, current_price: float, indicators: Dict[str, float],
                          recent_prices: List[float], recent_volumes: List[float]) -> Tuple[bool, str]:
        """
    ...
    def get_position_adjustment(self, base_position_size: float, current_price: float,
                              indicators: Dict[str, float], recent_prices: List[float],
                              recent_volumes: List[float]) -> Tuple[float, str]:
        """
        ...
        try:
            ...
            reasoning_list = entry_strategy.get('strategy_reasoning', ['Standard sizing'])
    ...
    def analyze_historical_data(self, price_data: List[Dict]):
        ...
        patterns = self.peak_system.analyze_price_history(prices, volumes, timestamps, indicators)
