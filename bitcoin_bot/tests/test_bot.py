...
from enhanced_trading_bot import EnhancedTradingBot, AdaptiveLearningEngine
...
class TestOrderManager(unittest.TestCase):
    ...
    def test_order_timeout(self):
        ...
        self.mock_api.fetch_order.return_value = {
            'status': 'cancelled',
            'filled': 0
        }
    ...
    def test_complete_trading_cycle(self, mock_eur, mock_btc, mock_price):
        ...
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            data_manager = DataManager(
                os.path.join(tmpdir, "prices.json"),
                os.path.join(tmpdir, "logs.csv")
            )
            ...
            mock_onchain.get_onchain_signals.return_value = {
                'fee_rate': 10, 'netflow': -5000, 'volume': 10000, 'old_utxos': 5
            }
            ...
            mock_api.create_limit_order.return_value = {'id': 'test_order', 'status': 'open'}
            
            # Execute strategy
            with patch('indicators.fetch_enhanced_news', return_value=[]):
                with patch('indicators.calculate_enhanced_sentiment', 
                          return_value={'sentiment': 0.1, 'risk_off_probability': 0.2}):
                    bot.execute_strategy()
            
            # Verify execution
            self.assertTrue(mock_api.create_limit_order.called or 
                          mock_api.fetch_order_book.called)
...
class TestPerformance(unittest.TestCase):
    ...
    def test_indicator_calculation_performance(self):
        ...
        rsi_time = timeit.timeit(
            lambda: calculate_rsi(prices),
            number=100
        )
    ...
    def test_memory_usage_price_history(self):
        ...
        # Add 10000 candles
        large_ohlc = [
            [1609459200 + i*60, 50000, 51000, 49000, 50500, 100]
