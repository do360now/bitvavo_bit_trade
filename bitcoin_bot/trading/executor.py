...
class TradeExecutor:
    ...
    def execute_trade(self, volume: float, side: str, price: float) -> bool:
        ...
        try:
            ...
            try:
                ...
                if market_info:
                    ...
                    if min_amount and volume < min_amount:
                        logger.error(f"Order amount {volume:.8f} BTC below minimum {min_amount:.8f} BTC")
                        return False
                        
                    if min_cost and order_value < min_cost:
                        logger.error(f"Order value €{order_value:.2f} below minimum €{min_cost:.2f}")
                        return False
                        
                    logger.debug(f"Order meets requirements: {volume:.8f} BTC (min: {min_amount:.8f}), €{order_value:.2f} (min: €{min_cost:.2f})")
            except Exception as e:
            ...
            if order and order.get('id'):
                logger.info(f"✅ {side.upper()} order placed: {volume:.8f} BTC at €{price:.2f}, Order ID: {order['id']}")
                return True
            else:
    ...
    def get_total_btc_balance(self) -> Optional[float]:
        ...
        try:
            ...
            logger.debug(f"BTC balance - Total: {total:.8f}, Free: {free:.8f}, Used: {used:.8f}")
    ...
    def get_ohlc_data(self, pair: str = None, interval: str = '15m', since: int = None, limit: int = 100):
            ...
            if ohlcv:
                ...
                for i, candle in enumerate(ohlcv):
                    try:
                        ...
                        if i < 5:
                            logger.info(f"Candle {i}: Raw timestamp {timestamp_ms} -> {timestamp} ({datetime.fromtimestamp(timestamp)})")
                        ...
                        if timestamp < min_valid:
                            logger.debug(f"Skipping old timestamp: {timestamp} ({datetime.fromtimestamp(timestamp)})")
                            continue
                            
                        if timestamp > max_valid:
                            logger.debug(f"Skipping future timestamp: {timestamp} ({datetime.fromtimestamp(timestamp)})")
                            continue
                ...
                if formatted_data:
                    ...
                    logger.info(f"Time range: {datetime.fromtimestamp(formatted_data[0][0])} to {datetime.fromtimestamp(formatted_data[-1][0])}")
                    logger.info(f"Price range: €{min(c[4] for c in formatted_data):.2f} to €{max(c[4] for c in formatted_data):.2f}")
                else:
    ...
    def cancel_order(self, order_id: str) -> bool:
        ...
        try:
            ...
            logger.info(f"Order {order_id} cancelled successfully with operatorId {operator_id}")
            return True
        except Exception as e:
