...
class OrderManager:
        ...
        try:
            ...
            for order_id in list(self.pending_orders.keys()):
                if order_id not in exchange_order_ids:
                    logger.warning(
                        f"⚠️ Order {order_id} is pending locally but not on exchange"
                    )
                    ...
                    if final_status:
                        ...
                        self._mark_order_as_cancelled(
                            order_id, "Not found on exchange during sync"
                        )
            ...
            logger.info(
                f"✅ Sync complete: {len(self.pending_orders)} orders currently pending"
            )
        ...
        order_info.update(
            {
                ...
                "average_price": final_status.get(
                    "average", order_info.get("price", 0)
                ),
    ...
    def _mark_order_as_cancelled(self, order_id: str, reason: str):
        ...
        order_info = self.pending_orders.get(order_id, {}).copy()
        order_info.update(
            {"status": "cancelled", "cancelled_at": time.time(), "reason": reason}
        )
        self.cancelled_orders[order_id] = order_info
        ...
        try:
            ...
            try:
                order = self.bitvavo_api.create_limit_order(
                    ...
                    price=price,
                    params=(
                        {"operatorId": operator_id, "postOnly": post_only}
                        if post_only
                        else {"operatorId": operator_id}
                    ),
                )
    ...
    def check_and_update_orders(self) -> Dict[str, List[str]]:
        ...
        results = {"filled": [], "cancelled": [], "partial": [], "still_pending": []}
    ...
    def _check_single_order(self, order_id: str) -> Optional[str]:
        ...
        try:
            ...
            if status in ["closed", "filled"]:
                ...
                logger.info(
                    f"✅ Order {order_id} FILLED: {filled_amount:.8f} BTC @ €{order_info['average_price']:.2f}"
                )
                return "filled"
                ...
                logger.info(
                    f"📊 Order {order_id} PARTIAL: {filled_amount:.8f}/{order_info['volume']:.8f} BTC"
                )
                return "partial"
    ...
    def _periodic_sync_check(self):
        ...
        try:
            ...
            for order_id in list(self.pending_orders.keys()):
                if order_id not in exchange_order_ids:
                    logger.info(
                        f"🔍 Order {order_id} no longer on exchange, investigating..."
                    )
                    final_status = self._get_final_order_status(order_id)
                    if final_status:
                        self._process_completed_order(order_id, final_status)
    ...
    def _load_order_history(self):
        ...
        try:
            if os.path.exists(self.order_history_file):
                ...
                logger.info(
                    f"Loaded order history: {len(self.filled_orders)} filled, {len(self.cancelled_orders)} cancelled"
                )
        except Exception as e:
    ...
    def _save_order_history(self):
        ...
        try:
            ...
            logger.debug("Saved order history")
        except Exception as e:
    ...
    def cleanup_old_orders(self, days: int = 30):
        ...
        old_cancelled = [
            order_id
            for order_id, order_info in self.cancelled_orders.items()
            if order_info.get("cancelled_at", order_info.get("timestamp", 0))
            < cutoff_time
        ]
        ...
        if old_filled or old_cancelled:
            logger.info(
                f"Cleaned up {len(old_filled)} old filled and {len(old_cancelled)} old cancelled orders"
            )
            self._save_order_history()
