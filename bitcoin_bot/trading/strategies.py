...
class BaseStrategy(ABC):
    ...
    @abstractmethod
    def generate_signal(
        self, indicators: MarketIndicators, analysis: Dict[str, Any]
    ) -> TradingSignal:
        """
    ...
    def should_trade(self, indicators: MarketIndicators) -> Tuple[bool, str]:
        ...
        if self.state.last_action_time:
            time_since_last = (
                datetime.now() - self.state.last_action_time
            ).total_seconds()
        ...
        if executed and signal.action != TradingAction.HOLD:
            ...
            if signal.action == TradingAction.BUY:
                ...
                if self.state.average_entry_price > 0:
                    pnl = (
                        signal.price - self.state.average_entry_price
                    ) * signal.volume
                    self.total_pnl += pnl
...
class DCAStrategy(BaseStrategy):
    ...
    def analyze(self, indicators: MarketIndicators) -> Dict[str, Any]:
        ...
        if self.price_threshold:
            analysis["price_below_threshold"] = (
                indicators.current_price <= self.price_threshold
            )
    ...
    def generate_signal(
        ...
        if (
            analysis["should_buy"]
            and analysis["price_below_threshold"]
            and not analysis["accumulation_complete"]
        ):
...
class GridStrategy(BaseStrategy):
    ...
    def __init__(self, config: StrategyConfig):
        ...
        self.grid_spacing_percentage = config.params.get(
    ...
    def generate_signal(
        ...
        if analysis["nearest_buy_level"]:
            ...
            if buy_level not in analysis["active_buy_grids"]:
                if (
                    indicators.current_price <= buy_level * 1.01
                ):  # Within 1% of grid level
                    action = TradingAction.BUY
        ...
        if analysis["nearest_sell_level"] and action == TradingAction.HOLD:
            ...
            if sell_level not in analysis["active_sell_grids"]:
                if (
                    indicators.current_price >= sell_level * 0.99
                ):  # Within 1% of grid level
...
class MomentumStrategy(BaseStrategy):
    ...
    def analyze(self, indicators: MarketIndicators) -> Dict[str, Any]:
        ...
        price_history = self.state.custom_state.get(
            "price_history", deque(maxlen=self.momentum_period * 2)
        )
        volume_history = self.state.custom_state.get(
            "volume_history", deque(maxlen=self.momentum_period * 2)
        )
        ...
        if len(price_history) >= self.momentum_period:
            ...
            older_prices = list(price_history)[
                -self.momentum_period * 2 : -self.momentum_period
            ]
            ...
            if older_prices:
                ...
                analysis["strength"] = min(
                    1.0, abs(momentum) / (self.momentum_threshold * 3)
                )
            ...
            if self.trend_filter:
                analysis["trend_aligned"] = (
                    analysis["momentum_direction"] == "bullish"
                    and indicators.market_regime == "uptrend"
                ) or (
                    analysis["momentum_direction"] == "bearish"
                    and indicators.market_regime == "downtrend"
                )
...
class MeanReversionStrategy(BaseStrategy):
    ...
    def __init__(self, config: StrategyConfig):
        ...
        self.deviation_threshold = config.params.get(
    ...
    def analyze(self, indicators: MarketIndicators) -> Dict[str, Any]:
        ...
        if indicators.ma_long > 0:
            deviation = (
                indicators.current_price - indicators.ma_long
            ) / indicators.ma_long
            ...
            if self.use_bollinger and indicators.bollinger_middle > 0:
                ...
                if band_width > 0:
                    z_score = (
                        indicators.current_price - indicators.bollinger_middle
                    ) / (band_width / 4)
                    analysis["z_score"] = z_score

                    # Bollinger position (0 = lower band, 1 = upper band)
                    analysis["bollinger_position"] = (
                        (indicators.current_price - indicators.bollinger_lower)
                        / band_width
                        if band_width > 0
                        else 0.5
                    )
        ...
        if abs(analysis["z_score"]) > self.deviation_threshold:
            ...
            analysis["reversion_direction"] = (
                "up" if analysis["z_score"] < 0 else "down"
            )
        ...
        if analysis["reversion_expected"]:
            ...
            if analysis["reversion_direction"] == "up":
                buy_conditions = [
                    ...
                    (
                        analysis["bollinger_position"] < 0.2
                ...
                if all(buy_conditions):
                    ...
                    reasoning.append(
                        f"Price {analysis['deviation_from_mean']:.1%} below mean"
                    )
            ...
            # Sell signal - price above mean and expected to revert down
                ...
                sell_conditions = [
                    analysis["z_score"] > self.deviation_threshold,
                    (
                        analysis["bollinger_position"] > 0.8
                        if self.use_bollinger
                        else True
                    ),
                ...
                if all(sell_conditions):
                    ...
                    confidence = min(0.9, 0.5 + abs(analysis["z_score"]) * 0.1)
                    reasoning.append(
                        f"Price {analysis['deviation_from_mean']:.1%} above mean"
                    )
                    reasoning.append(f"Z-score: {analysis['z_score']:.2f}")
...
class BreakoutStrategy(BaseStrategy):
    ...
    def analyze(self, indicators: MarketIndicators) -> Dict[str, Any]:
        ...
        price_history = self.state.custom_state.get(
            "price_history", [indicators.current_price]
        )
        volume_history = self.state.custom_state.get(
            "volume_history", [indicators.current_volume]
        )

        if len(price_history) >= 20:
            # Identify levels
            support, resistance = self._identify_levels(
            ...
            if analysis["nearest_resistance"]:
                if (
                    current_price > analysis["nearest_resistance"] * 1.01
                ):  # 1% above resistance
                    analysis["breakout_type"] = "resistance"
                    analysis["breakout_strength"] = (
                        current_price - analysis["nearest_resistance"]
                    ) / analysis["nearest_resistance"]

            if analysis["nearest_support"]:
                if (
                    current_price < analysis["nearest_support"] * 0.99
                ):  # 1% below support
                    analysis["breakout_type"] = "support"
                    analysis["breakout_strength"] = (
                        analysis["nearest_support"] - current_price
            ...
            if len(volume_history) >= 10:
                ...
                analysis["volume_confirmed"] = (
                    recent_volume > avg_volume * self.volume_increase_threshold
                )

            # Detect consolidation (low volatility before breakout)
            if len(price_history) >= 20:
                recent_volatility = np.std(price_history[-10:]) / np.mean(
                    price_history[-10:]
                )
                older_volatility = np.std(price_history[-20:-10]) / np.mean(
                    price_history[-20:-10]
                )
                analysis["consolidation_detected"] = (
                    recent_volatility < older_volatility * 0.7
                )
    ...
    def generate_signal(
        ...
        if analysis["breakout_type"]:
            ...
            if analysis["breakout_type"] == "resistance":
                if analysis["volume_confirmed"]:
                    ...
                    reasoning.append(
                        f"Resistance breakout at €{analysis['nearest_resistance']:.0f}"
                    )
                    reasoning.append("Volume surge confirmed")
            ...
            # Support breakdown - sell signal
                ...
                if analysis["volume_confirmed"]:
                    ...
                    confidence = min(0.9, 0.6 + analysis["breakout_strength"] * 2)
                    reasoning.append(
                        f"Support breakdown at €{analysis['nearest_support']:.0f}"
                    )
        ...
        if action != TradingAction.HOLD:
            ...
            recent_attempts = self.state.custom_state.get(
                "breakout_attempts", deque(maxlen=10)
            )
            failed_attempts = sum(
                1 for attempt in recent_attempts if not attempt["successful"]
            )
        ...
        return TradingSignal(
            ...
            reasoning=reasoning,
            risk_level=(
                RiskLevel.HIGH if analysis["breakout_type"] else RiskLevel.MEDIUM
            ),
        )
...
class HybridAdaptiveStrategy(BaseStrategy):
    ...
    def __init__(self, config: StrategyConfig):
        ...
        self.strategies = {
            ...
            "mean_reversion": MeanReversionStrategy(
                self._create_sub_config("mean_reversion")
            ),
    ...
    def _create_sub_config(self, strategy_name: str) -> StrategyConfig:
        ...
        sub_config = StrategyConfig(
            ...
            * 0.5,  # Reduce for each sub-strategy
        ...
        for name, signal in signals.items():
            ...
            if signal.action == TradingAction.BUY:
                ...
                combined_reasoning.extend(
                    [f"[{name}] {r}" for r in signal.reasoning[:1]]
                )
            elif signal.action == TradingAction.SELL:
...
class StrategyFactory:
    ...
    @staticmethod
    def create_strategy(
        strategy_type: StrategyType, config: Optional[StrategyConfig] = None
    ) -> BaseStrategy:
        """
        ...
        default_configs = {
            StrategyType.DCA: StrategyConfig(
                name="DCA Strategy",
                params={"buy_interval_hours": 24, "buy_amount_percentage": 0.10},
            ),
            StrategyType.GRID: StrategyConfig(
                name="Grid Strategy",
                params={"grid_levels": 10, "grid_spacing_percentage": 0.02},
            ),
            ...
            StrategyType.BREAKOUT: StrategyConfig(
                ...
                params={"lookback_period": 100, "confirmation_bars": 3},
            ),
            StrategyType.HYBRID: StrategyConfig(name="Hybrid Adaptive Strategy"),
        }
