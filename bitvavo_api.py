...
class SecurityManager:
    ...
    def generate_signature(
        ...
        signature = hmac.new(
            self._api_secret, message.encode("utf-8"), hashlib.sha256
        ).hexdigest()
        return signature
...
class RateLimiter:
    ...
    async def acquire(self) -> None:
        ...
        async with self._lock:
            ...
            self.tokens = min(
                self.burst_capacity, self.tokens + elapsed * self.requests_per_second
            )
...
class EnhancedBitvavoAPI:
    ...
    async def _make_request(
        ...
        if authenticated:
            ...
            signature = self.security_manager.generate_signature(
                timestamp, method, f"/v2/{endpoint}", body
            )
    ...
    # Convenient async wrapper methods for bot integration
    async def get_ticker_async(self, symbol: str) -> Dict[str, Any]:
        """Get ticker data - wrapper for bot"""
        try:
            result = await self._make_request(
                "GET", f"ticker/price", {"market": symbol}
            )
            return {"price": float(result.get("price", 0))}
        except Exception as e:
    ...
    async def get_order_book_async(
        ...
        """Get order book - wrapper for bot"""
        try:
            result = await self._make_request("GET", f"{symbol}/book", {"depth": depth})
            return result
        except Exception as e:
    ...
    async def get_ohlcv_async(
        self, symbol: str, interval: int = 3600
    ) -> List[List[float]]:
        """Get OHLCV data - not directly available from Bitvavo, returns empty
        
        Bitvavo doesn't provide a candles/OHLCV endpoint via their REST API.
        The bot builds candles from price history instead.
        """
        logger.debug(
            f"Bitvavo REST API does not provide OHLCV data. "
            f"Using price history instead."
        )
        return []

    async def get_balance_async(self) -> Optional[Dict[str, Dict[str, float]]]:
        """Get account balance - wrapper for bot"""
        try:
            result = await self._make_request("GET", "balance", authenticated=True)
            # Parse Bitvavo balance format
            balances = {}
            for item in result:
                ...
                balances[symbol] = {
                    ...
                    "inOrder": float(item.get("inOrder", 0)),
                }
            return balances
        except Exception as e:
    ...
    async def place_order_async(
        ...
        """Place order - wrapper for bot"""
        try:
            result = await self._make_request(
                "POST", "order", data=order_data, authenticated=True
            )
            return result
        except Exception as e:
    ...
    async def get_order_async(
        self, order_id: str, market: str = "BTC-EUR"
    ) -> Optional[Dict[str, Any]]:
        """Get order status - wrapper for bot"""
        try:
            result = await self._make_request(
                "GET",
                f"order",
    ...
    async def cancel_order_async(
        self, market: str, order_id: str
    ) -> Optional[Dict[str, Any]]:
        """Cancel order - wrapper for bot"""
        try:
            result = await self._make_request(
                "DELETE",
                "order",
...
def authenticate_bitvavo():
    ...
    try:
        ...
        if test_connection(exchange):
            ...
            logger.warning(
                "Authentication test failed, proceeding with unverified connection"
            )
            return exchange
