...
class AdaptiveLearningEngine:
    ...
    def __init__(self):
        ...
        self.feature_names = [
            'rsi', 'macd', 'signal', 'bb_position', 'volume_ratio', 
            'price_momentum', 'volatility', 'sentiment', 'risk_off_prob',
            'hour_of_day', 'day_of_week', 'recent_performance'
        ]
    ...
    def extract_features(self, indicators_data: Dict, current_time: datetime = None) -> np.array:
        ...
        # Volume and momentum
        volume_ratio = min(2.0, indicators_data.get('volume_ratio', 1.0)) / 2.0  # Normalize
        price_momentum = np.tanh(indicators_data.get('price_momentum', 0) / 0.1)  # Bounded
        volatility = min(1.0, indicators_data.get('volatility', 0.02) / 0.1)  # Normalize
        
        # Sentiment and news
        sentiment = (indicators_data.get('sentiment', 0) + 1) / 2  # Convert -1,1 to 0,1
        risk_off_prob = indicators_data.get('news_analysis', {}).get('risk_off_probability', 0)
        ...
        # Recent performance
        recent_performance = (indicators_data.get('recent_win_rate', 0.5) - 0.5) * 2  # Convert to -1,1
    ...
    def train_on_historical_data(self, bot_logs_df: pd.DataFrame):
        ...
        for i in range(len(bot_logs_df) - 1):
            ...
            if row.get('buy_decision') == 'True':
                ...
                profitable = 1 if next_price > current_price * 1.002 else 0  # 0.2% threshold
...
class MarketRegimeDetector:
    ...
    def detect_regime(self, prices: List[float], volumes: List[float]) -> Dict[str, any]:
        """Detect current market regime"""
        if len(prices) < 50:
            return {"regime": "unknown", "confidence": 0.0, "volatility_regime": "normal"}
    ...
    def _analyze_market_structure(self, prices: np.array) -> Dict[str, any]:
        ...
        if len(prices) < 20:
            return {"support_levels": [], "resistance_levels": [], "breakout_potential": 0.0}
...
class AdaptivePositionSizer:
    ...
    def calculate_position_size(self, signal: TradeSignal, balance: float, 
                              market_regime: Dict, recent_performance: Dict) -> float:
        ...
        final_size = (base_size * confidence_multiplier * urgency_multiplier * 
                     performance_multiplier * regime_multiplier * risk_multiplier)
...
class EnhancedTradingBot:
    ...
    def generate_enhanced_signal(self, indicators_data: Dict) -> TradeSignal:
        ...
        basic_action = self.original_bot.enhanced_decide_action_with_risk_override(indicators_data)
        
        # Get ML prediction
        ml_fail_prob, ml_success_prob = self.learning_engine.predict_trade_success(indicators_data)
        ...
        confidence = self._calculate_confidence(indicators_data, ml_success_prob, market_regime)
        ...
        if ml_success_prob < 0.3 and basic_action in ['buy', 'sell']:
            ...
            if self._detect_strong_signals(indicators_data, market_regime):
                ...
                reasoning = [f"ML model predicts high success rate ({ml_success_prob:.2%})", "Strong technical signals detected"]
            else:
                action = basic_action
                reasoning = ["Following conservative approach despite ML confidence"]
        else:
            action = basic_action
            reasoning = [f"Following enhanced risk system decision"]
        
        # Add detailed reasoning
        reasoning.extend(self._generate_reasoning(indicators_data, market_regime, ml_success_prob))
        
        # Calculate expected duration
        expected_duration = self._estimate_trade_duration(indicators_data, market_regime)
    ...
    def _calculate_confidence(self, indicators_data: Dict, ml_success_prob: float, 
        ...
        sentiment = abs(indicators_data.get('sentiment', 0))  # Strong sentiment either way
    ...
    def _calculate_urgency(self, indicators_data: Dict, market_regime: Dict) -> float:
        ...
        # Breakout potential
        breakout_potential = market_regime.get('structure', {}).get('breakout_potential', 0.0)
    ...
    def _detect_strong_signals(self, indicators_data: Dict, market_regime: Dict) -> bool:
    ...
    def _generate_reasoning(self, indicators_data: Dict, market_regime: Dict, 
                          ml_success_prob: float) -> List[str]:
    ...
    def _estimate_trade_duration(self, indicators_data: Dict, market_regime: Dict) -> int:
    ...
    def execute_enhanced_strategy(self):
        ...
        try:
            ...
            self.original_bot.price_history = self.original_bot.price_history[-400:]  # Keep last 400 prices
            
            # Get fresh OHLC data
            ohlc = self.original_bot.trade_executor.get_ohlc_data(pair="BTC/EUR", interval='15m', since=int(time.time() - 7200))
            if ohlc:
            ...
            self.original_bot.performance_tracker.update_equity(btc_balance, eur_balance, current_price)
            performance_report = self.original_bot.performance_tracker.generate_performance_report()
            ...
            indicators_data = {
                ...
                'volume_ratio': current_volume / np.mean(volumes[-20:]) if len(volumes) >= 20 else 1.0,
            ...
            if signal.action in ['buy', 'sell']:
                ...
                if signal.action == 'buy':
                    position_btc = min(position_size / current_price, eur_balance * 0.9 / current_price)
                else:
                ...
                if position_btc < min_trade_size:
                    signal.action = 'hold'
                    signal.reasoning.append(f"Position size too small: {position_btc:.8f} BTC")
            else:
                position_btc = 0
            
            # Enhanced logging
            self._log_enhanced_decision(signal, market_regime, indicators_data, position_btc)
            
            # Execute the trade with enhanced logic
            if signal.action in ['buy', 'sell'] and position_btc > 0:
                success = self._execute_enhanced_trade(signal, position_btc, current_price)
                if success:
    ...
    def _calculate_recent_performance(self) -> Dict[str, float]:
        ...
        try:
            ...
            trades = recent_df[(recent_df['side'].isin(['buy', 'sell'])) & 
            ...
            for i in range(len(trades) - 1):
                ...
                next_price = trades.iloc[i + 1]['price'] if i < len(trades) - 1 else current_trade['price']
                
                if current_trade['side'] == 'buy':
                    trade_return = (next_price - current_trade['price']) / current_trade['price']
                elif current_trade['side'] == 'sell':
                    trade_return = (current_trade['price'] - next_price) / current_trade['price']
                else:
                    continue
    ...
    def _execute_enhanced_trade(self, signal: TradeSignal, volume: float, current_price: float) -> bool:
        ...
        try:
            ...
            optimal_price = self.original_bot.trade_executor.get_optimal_price(order_book, signal.action)
            ...
            if self.original_bot.order_manager:
                order_id = self.original_bot.order_manager.place_limit_order_with_timeout(
                    ...
                    price=optimal_price,
                    timeout=timeout,
                    post_only=(signal.urgency < 0.5)  # Use post-only for non-urgent trades
                )
                
                if order_id:
                    self.original_bot.last_trade_time = time.time()
                    print(f"✅ {signal.action.upper()} order placed: {volume:.8f} BTC at €{optimal_price:.2f}")
                    print(f"   Confidence: {signal.confidence:.1%}, Risk: {signal.risk_level}")
                ...
                success = self.original_bot.trade_executor.execute_trade(volume, signal.action, optimal_price)
                if success:
                    print(f"✅ {signal.action.upper()} executed: {volume:.8f} BTC at €{optimal_price:.2f}")
                    return True
                else:
    ...
    def _log_enhanced_decision(self, signal: TradeSignal, market_regime: Dict, 
                             indicators_data: Dict, position_btc: float):
    ...
    def get_performance_summary(self) -> Dict:
        ...
        try:
            ...
            basic_report = self.original_bot.performance_tracker.generate_performance_report()
            ...
            ml_status = {
                ...
                'confidence_in_predictions': 'High' if self.learning_engine.is_trained else 'Building'
            }
    ...
    def print_enhanced_status(self):
        ...
        try:
            ...
            print(f"🌊 Volatility: {regime['volatility_regime']} ({regime['volatility']:.1%})")
            
            # ML Status
            print(f"🧠 ML Model: {'Trained' if self.learning_engine.is_trained else 'Learning'}")
...
def test_enhanced_bot():
    ...
    test_prices = list(range(100, 150)) + list(range(150, 120, -1))  # Trend then reversal
    ...
    size = position_sizer.calculate_position_size(test_signal, 1000, regime, {'win_rate': 0.6})
