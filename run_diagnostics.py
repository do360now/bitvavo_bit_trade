...
def check_file_exists(filepath, description):
    ...
    if os.path.exists(filepath):
        ...
        print(f"✅ {description}: {filepath} (size: {size} bytes, modified: {mod_time})")
        return True
    else:
...
def analyze_order_history():
    ...
    try:
        ...
        print(f"⏰ Last updated: {datetime.fromtimestamp(last_updated) if last_updated else 'Never'}")
        ...
        if recent_filled:
            ...
            for order in recent_filled[-5:]:  # Last 5
                ...
                print(f"  {side.upper()}: {volume:.8f} BTC @ €{price:.2f} at {filled_time.strftime('%H:%M:%S')}")
...
def analyze_trade_session():
    ...
    try:
        ...
        if session_date != current_date:
            print(f"⚠️ Session date is old - will reset on next bot run")
...
def analyze_bot_logs():
    ...
    try:
        ...
        sell_trades = df[df['sell_decision'].astype(str).str.lower().isin(['true', '1'])]
        ...
        if len(recent_logs) > 0:
            print(f"📊 Recent price range: €{recent_logs['price'].min():.0f} - €{recent_logs['price'].max():.0f}")
            
            recent_buys = recent_logs[recent_logs['buy_decision'].astype(str).str.lower().isin(['true', '1'])]
            recent_sells = recent_logs[recent_logs['sell_decision'].astype(str).str.lower().isin(['true', '1'])]
...
def test_api_connectivity():
    ...
    try:
        ...
        print(f"💰 Total portfolio value: €{eur_balance + (btc_balance * current_price):.2f}")
    ...
    try:
        ...
        if pending:
            ...
            for order_id, order_info in pending.items():
                ...
                print(f"  {side.upper()}: {volume:.8f} BTC @ €{price:.2f} (age: {age/60:.1f}min)")
        ...
        if filled_24h:
            ...
            for order_id, order_info in list(filled_24h.items())[-3:]:
                ...
                print(f"  {side.upper()}: {volume:.8f} BTC @ €{price:.2f} at {filled_time.strftime('%H:%M')}")
...
def generate_recommendations():
    ...
    if not os.path.exists("./order_history.json"):
        recommendations.append("🔧 Bot hasn't been running long enough to generate orders")
        recommendations.append("   → Let the bot run for at least 30 minutes to see activity")
    ...
    if os.path.exists("./bot_logs.csv"):
        try:
            ...
            if len(recent) == 0:
                ...
                recommendations.append("   → Bot might be in 'hold' mode due to market conditions")
        except:
            pass
