...
try:
    ...
    from bot import TradingBot as CoreBot, BotConfiguration, MarketIndicators, TradingSignal, TradingAction, RiskLevel
except ImportError:
    # Fallback for different structure
    from core.bot import TradingBot as CoreBot, BotConfiguration, MarketIndicators, TradingSignal, TradingAction, RiskLevel
...
try:
    from peak_detection import PeakAvoidanceSystem, PeakDetectionConfig
except ImportError:
...
try:
    from strategies import StrategyFactory, StrategyType, StrategyConfig
except ImportError:
...
class UnifiedTradingBot:
    ...
    def _initialize_strategies(self):
        ...
        try:
            ...
            for name, config in strategy_configs.items():
                try:
                    strategy_type = getattr(StrategyType, name.upper() if name != 'hybrid' else 'HYBRID')
        ...
        if self.peak_system:
            try:
                ...
                if len(prices) >= 50:
                    ...
                    indicators.peak_recommendation = analysis.get('recommended_action', 'normal_entry')
        ...
        for name, strategy in self.strategies.items():
            try:
                ...
                if should_trade:
                    ...
                    logger.debug(f"{name} strategy signal: {signal.action.value} (conf: {signal.confidence:.2f})")
    ...
    def _combine_signals(self, signals: List[Tuple[str, TradingSignal, float]], indicators: MarketIndicators) -> TradingSignal:
        """
        ...
        if (self.peak_system and 
            indicators.peak_probability and 
            indicators.peak_probability > 0.7 and 
            action == TradingAction.BUY):
            
            logger.warning(f"Peak override: Changing BUY to HOLD (peak prob: {indicators.peak_probability:.1%})")
            action = TradingAction.HOLD
            confidence *= 0.7
            all_reasoning.insert(0, f"Peak detection override ({indicators.peak_probability:.1%})")
                ...
                if success:
                    ...
                    if self.ml_engine and signal.action != TradingAction.HOLD:
                        try:
                            ...
                            logger.debug("Trade executed - will update ML model with outcome later")
    ...
    def get_comprehensive_status(self) -> Dict[str, Any]:
        ...
        unified_status = {
            ...
            'components': {
                ...
                'ml_engine_trained': self.ml_engine.is_trained if self.ml_engine else False,
    ...
    def print_comprehensive_status(self):
        ...
        try:
            ...
            print(f"\n{'='*80}")
            print(f"🤖 UNIFIED BITCOIN TRADING BOT STATUS")
            print(f"{'='*80}")
            
            # Session info
            session = status['unified_bot']
            uptime = (datetime.now() - datetime.fromisoformat(session['session_start'])).total_seconds() / 3600
            ...
            print(f"\n🔧 COMPONENTS:")
            print(f"   🧠 ML Engine: {'✅ Trained' if components['ml_engine_trained'] else '❌ Not trained' if components['ml_engine_enabled'] else '⏸️  Disabled'}")
            print(f"   📊 Peak Detection: {'✅ Active' if components['peak_detection_enabled'] else '⏸️  Disabled'}")
            print(f"   🎯 Active Strategies: {components['strategy_count']} ({', '.join(components['active_strategies'])})")
            ...
            if status['strategy_performance']:
                ...
                for name, perf in status['strategy_performance'].items():
                    ...
                    print(f"   {name.title()}: {win_rate:.1%} win rate ({trades} trades)")
